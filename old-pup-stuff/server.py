...
class VideoTransformTrack(MediaStreamTrack):
    ...
    async def recv(self):
        ...
        if self.transform == "cartoon":
            ...
            M = cv2.getRotationMatrix2D((cols / 2, rows / 2), frame.time * 45, 1) # TODO: check if we can just use frame.time instead of timeStep
...
async def offer(request):
    ...
    # send answer
    answer = await pc.createAnswer() # complement to pc.createOffer() in the js file
    await pc.setLocalDescription(answer) # answer set to pc object
    # pc object now has session descriptions for both peers

    # answer sent back to frontend as json of response
    return web.Response(
